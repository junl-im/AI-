# AI 쇼츠 제작 스튜디오 v1.5.26

## Adaptive Cache Quota · Integrity Audit Backoff · Protected Recovery

- 영구 분석 캐시는 브라우저 저장소 사용률을 확인해 정상 8개·16MiB, 경고 시 60%, 위험 시 최소 2개·4MiB로 한도를 자동 조정합니다.
- quota 오류가 발생하면 만료·LRU 항목을 정리한 뒤 안전하게 재시도하며, 진단 화면에서 항목별 예상 크기와 사용 시각을 확인하고 선택 삭제할 수 있습니다.
- 서비스워커는 순환 SHA-256 감사 결과를 최대 40개 보관하고 반복 실패 자산에 5분~6시간 지수 백오프를 적용합니다.
- 온라인 복귀 시 미뤄진 감사가 자동 재개되며 최근 감사·복구·백오프 상태를 개인정보 없는 JSON으로 저장할 수 있습니다.
- 세션 패널에서 선택한 정상 스냅샷을 회전 정리되지 않는 중요 백업으로 보호하고, 복원 전에 프로젝트 요약을 확인할 수 있습니다.
- build key는 `1.5.26-adaptive-cache-audit-protected-recovery`입니다.
- 자동 QA **208/208**, Chromium 4개 viewport 오류·Promise 거절·콘솔 오류·가로 overflow **0건**을 통과했습니다.
