# AI 쇼츠 제작 스튜디오 v1.5.25

## Persistent Analysis Cache · Selective Recovery · Rotating Integrity Audit

- 메모리 분석 캐시에 선택적 IndexedDB 영구 계층을 추가했습니다. 브라우저 지원 여부, TTL, 앱 버전 namespace, 항목 수 8개, 총 16MiB 제한을 모두 적용합니다.
- 분석 캐시 진단은 적중·미적중·만료·퇴출·지문 비용을 집계하며 파일명·원본 경로·원시 캐시 키를 포함하지 않는 JSON으로 내보낼 수 있습니다.
- 세션 패널에서 기본 기록과 각 순환 백업의 저장 시각·압축률·상태를 확인하고 원하는 시점을 직접 선택해 복원할 수 있습니다.
- 서비스워커는 유휴 시간마다 앱 셸 12개 파일을 순환 SHA-256 검사하고, 누락·HTTP 오류·본문 손상이 발견된 표본만 다시 내려받아 검증합니다.
- 페이지가 숨겨졌거나 오프라인이면 자동 검사를 미루며, 진단 화면에서 즉시 표본 검사를 실행할 수 있습니다.
- build key는 `1.5.25-persistent-analysis-selective-recovery-integrity-audit`입니다.
- 자동 QA **204/204**, Chromium 4개 viewport 오류·Promise 거절·콘솔 오류·가로 overflow **0건**을 통과했습니다.
