...
- Version: `0.11.27 · Field Device & MY VOICE Runtime Certification`
...
- Web CI correction: Actions run `32117983645`에서 lint는 통과했고 critical regression 65개 중 exit confirmation test 1건만 실패했습니다. 테스트가 실제 Back의 guard→base history state 이동을 생략한 것이 원인이며 0.11.27 전달본에서 test harness만 교정했습니다.
...
- 카카오 모바일 음성: Browser Speech preset 미리듣기는 최초 탭 안에서 직접 시작하고 1.8초 start watchdog으로 무음 멈춤을 해제합니다. 카카오 WebView가 음성을 제한하면 외부 브라우저 열기 안내를 제공합니다.
- 모바일 종료 확인: 첫 뒤로가기는 확인창만 열고 `계속 만들기`에서 guard를 다시 설치하며, `종료`는 base entry에서 한 번만 뒤로 이동합니다.
- 모바일 가로 타임라인: 1024px 미만에서도 ruler·clip·playhead가 같은 좌→우 시간축을 사용하고 트랙 라벨 공간을 줄여 화면 폭을 최대한 사용합니다.
- 재생 연결: 생성된 track의 명시적 재생 요청은 플레이어 버튼 상태와 즉시 동기화하며 native 재생 실패 시 오류 상태로 안전하게 복귀합니다.
- 다중 화자 Assist: 대본이 완전히 명확한 `화자: 대사` 형식일 때만 화자를 감지하고, 화자별 목소리를 제안한 뒤 사용자가 확인해야 실제 생성에 적용합니다.
- 화자 배정 기억: 승인한 최근 화자-목소리 조합은 화자 원문 대신 해시 키+voiceId만 최대 24건 저장하고 다음 대본에서 우선 제안합니다.
- 화자별 프로젝트 복원: 생성 프로젝트에 클립별 대사·voiceId·voiceName·job 순서를 저장해 다시 열어도 다중 화자 타임라인이 단일 목소리로 합쳐지지 않습니다.
- 이어서 만들기: 생성 중지 뒤 이미 완성된 음성은 그대로 두고 queued 대사만 `남은 대사 이어서 만들기` 한 번으로 재개합니다.
- One-Flow 장문 생성: 첫 대사 우선 재생 + 나머지 최대 2개 안전 병렬 처리 + 원문 순서 자동 복원
- 첫 문장 실제 대본 미리듣기, SRT/VTT 붙여넣기 자동 정리, Markdown 말하기용 정리, 생성 진행률/중지 제공
- 원플로우 더빙: 새 작업은 좌우 프로 패널을 접고 중앙에서 `목소리 → 대본 → 바로 더빙 → 듣기`까지 한 흐름으로 끝냅니다.
- 목소리 선택: 현재 선택 목소리 1개만 보이고 팝업에서 전체 프리셋의 잘 맞는 상황·장점·주의점과 미리듣기를 비교합니다. 대본 추천은 자동 적용하지 않습니다.
...
- 편집 이력 UX: 최근 20단계 bounded Undo/Redo를 제공하며 이동·재정렬·대사 수정·분할·쉼·삭제·일괄 목소리 변경을 `Ctrl/Cmd+Z`, `Ctrl/Cmd+Shift+Z`와 toolbar에서 되돌립니다.
...
- 엔진 routing trace: 장문 batch 완료 시 실제 엔진별 사용 횟수·전환 횟수·fallback 횟수를 집계해 제작 완료 메시지에 표시합니다.
