...
- 실제 Chromium에서 20초 WAV를 넣어 확인한 결과, 기존 `hyperflow-tabs`의 로컬 `active=file` 값이 새 진행 상태를 다시 덮어쓰는 문제를 발견해 외부 단일 소유자의 `data-active-flow-tab`을 우선 채택하도록 수정했습니다.
...
- 상단 버전 바로 옆에 CSS 기반 기기 아이콘과 `모바일 · PC 호환` 문구를 추가해 PC와 모바일 모두 표시합니다.
- 운영체제마다 모양이 달라지는 컬러 이모지를 줄이고 파일·추천·후보·미리보기·파형·컷·편집·저장을 `＋ ✦ ◆ ▶ ∿ ✂ ◫ ↓` 스튜디오 글리프로 통일했습니다.
- 렌더 큐, 후보 안내와 보조 액션도 같은 기호 체계로 정리했습니다.
- 모바일 업데이트 알림은 상단 메타를 가리지 않도록 하단 메뉴바 위로 이동했습니다.
- 합성 20초 WAV 브라우저 E2E에서 `파일 → 추천 → 후보 → 미리보기`의 메뉴 활성·패널 강조·화면 이동이 일치함을 확인했습니다.
- v1.2.4 전용 내비게이션 회귀 검사 `qa/navigation_focus_smoke.js`를 추가했습니다.
- 버전, 빌드 키와 서비스워커 캐시를 v1.2.4 / `1.2.4-navigation-focus`로 동기화했습니다.

## v1.2.3 - Shorts Pulse Hero

- 상단을 쇼츠의 세로 비율과 빠른 컷 흐름이 직접 보이는 `Shorts Pulse Hero`로 개편했습니다.
- 왼쪽에 큰 제품명과 0:00 → 0:15 컷 타임라인을 배치해 짧은 영상 제작 도구라는 성격을 명확히 했습니다.
- 오른쪽에는 장식 카드 대신 겹쳐진 9:16 프레임, HOOK·BEAT·CAPTION 큐와 단일 시작 액션을 배치했습니다.
- `새 쇼츠 만들기`는 기존 `fileInput`을 그대로 사용하며 원본을 브라우저 안에서만 처리한다는 안내를 유지합니다.
- 상단 메타를 `BUILD v1.2.3 · SHORTS-FIRST STUDIO`, `LOCAL RENDER · 9:16 READY`, `DESIGNED BY 곰같은여우`로 통일했습니다.
- `flow-command-bridge.js`가 로컬 상태 문구를 이모지 기반 호환성 문구로 덮어쓰던 동작을 제거하고 같은 값일 때 DOM을 다시 쓰지 않도록 했습니다.
- 모바일에서는 9:16 비주얼과 시작 액션을 숨기고 브랜드·설명·짧은 타임라인만 남겨 하단 메뉴바와 역할이 겹치지 않게 했습니다.
- 모션 감소와 저사양 모드에서는 타임라인 움직임과 배경 빛 효과를 중단합니다.
- 자동 QA 105/105, Chromium 런타임 감사에서 RAF 25회·Mutation 58회 고정, 오류 0건을 확인했습니다.
- 버전, 빌드 키와 서비스워커 캐시를 v1.2.3 / `1.2.3-shorts-pulse-hero`로 동기화했습니다.
