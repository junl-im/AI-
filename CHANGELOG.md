...
- IndexedDB 영구 분석 캐시가 `navigator.storage.estimate()` 결과에 따라 저장 한도를 자동 조정하도록 개선했습니다. 정상 상태는 8개·16MiB, 경고 상태는 기본 한도의 60%, 위험 상태는 최소 2개·4MiB를 사용합니다.
- quota 오류 발생 시 만료·LRU 항목을 적극 정리한 뒤 쓰기를 한 번 재시도하고, 정책 변경·정리·실패 이력을 개인정보 비노출 진단 이벤트로 기록합니다.
- 영구 분석 캐시 항목별 비식별 토큰·예상 바이트·생성/최근 사용 시각을 표시하고, 사용자가 선택한 항목 하나만 삭제할 수 있는 기능을 추가했습니다.
- 서비스워커 순환 무결성 감사 이력을 최대 40개 보관하고, 반복 실패 자산에는 5분부터 최대 6시간까지 지수 백오프를 적용합니다.
- 온라인 복귀 시 미뤄진 무결성 검사를 자동 재개하며, 최근 감사·복구·백오프·롤백 상태를 개인정보 없이 JSON으로 내보낼 수 있습니다.
- 세션 복원 목록에 회전 정리되지 않는 중요 보호 백업을 추가하고, 선택한 정상 스냅샷을 보호하거나 보호 해제할 수 있도록 했습니다.
- 복원 전 프로젝트 제목·추천 수·자막 수·선택 구간·압축 상태를 미리 확인할 수 있는 요약 패널을 추가했습니다.
- 신규 회귀 검사 4개를 추가해 자동 QA **208/208**, Chromium 4개 viewport 오류·Promise 거절·콘솔 오류·가로 overflow **0건**을 통과했습니다.
