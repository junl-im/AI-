...
- 방향키와 Shift+방향키로 크롭 위치를 정밀하게 이동하고 Enter로 저장할 수 있습니다.
- 직접 조작이 끝나면 현재 재생 위치에 크롭 키프레임을 자동 저장합니다.
- 키프레임 이동 경로와 현재 위치를 미니맵으로 표시하며 직전 조정을 되돌릴 수 있습니다.
- 수동 직접 편집은 화자·얼굴·모션 자동 추적보다 우선하고 프로젝트 스키마 v5에 그대로 저장됩니다.
- 실제 포인터·휠·키보드 조작, 30분 1080p 스마트 크롭, 4개 화면을 포함해 자동 품질 게이트 245개를 통과했습니다.

# v1.6.8

- 로컬 전사·자막의 발화 구간과 얼굴 움직임을 결합해 실제 말하는 사람을 우선 추적합니다.
- whisper.cpp·OpenAI 호환 전사의 화자 라벨을 보존하고, 라벨이 없으면 얼굴 활동량으로 화자를 추정합니다.
- 대화 장면에서 화자가 바뀌면 스마트 9:16 크롭도 해당 얼굴로 전환합니다.
- 수동 주 피사체 고정과 수동 크롭 키프레임은 자동 화자 전환보다 항상 우선합니다.
- 화자 연결 결과와 우선 추적 설정을 프로젝트 스키마 v5에 저장하며 v4 프로젝트를 안전하게 이관합니다.
- 실제 20초 MP4의 두 얼굴·두 화자 전환과 30분 1080p 스마트 크롭 경로를 검증했습니다.

# v1.6.7

- verified local browser vision model packs
- MediaPipe Tasks Vision runtime and face-model local installation
- SHA-256 verification before activation and corruption shutdown
- dedicated service-worker model cache bridge
- CPU(WASM), GPU, and automatic delegate selection
- compact smart-reframe model-pack controls

# v1.6.6

- 스마트 리프레임을 여러 인물 구분과 주 피사체 수동 고정으로 확장했습니다.
- 재생 위치별 X/Y/확대 크롭 키프레임 생성·삭제·전체 초기화를 추가했습니다.
- 장면 전환 경계에서 이전 장면 위치를 다음 장면으로 보간하지 않도록 보호합니다.
- 주 피사체와 크롭 키프레임을 프로젝트 JSON에 안전하게 저장·복원합니다.
- 좁은 카드에서 버튼 폭이 눌림 효과 중 줄어 클릭이 유실되던 문제를 수정했습니다.
- 영상 탐색 완료 이벤트가 사용자가 조정 중인 크롭 값을 덮어쓰지 않도록 편집 초안 소유권을 추가했습니다.
- 실제 20초 다중 인물 흐름과 30분 1080p 스마트 크롭 경로를 새 버전에서 재검증했습니다.

# v1.6.5

- 세로 프레임에 스마트 피사체 추적을 추가했습니다.
- 영상 움직임 분석에서 공간 중심·확산도·신뢰도를 계산해 외부 모델 없이 모션 추적선을 생성합니다.
- 브라우저 FaceDetector와 MediaPipe Face Detector adapter를 지원하며 감지 실패 시 모션 추적으로 자동 대체합니다.
- 추적점 평활화·시간 보간·최대 이동 제한과 자막 위치를 피하는 9:16 동적 크롭을 적용했습니다.
- 미리보기·재생·썸네일·최종 렌더가 동일한 추적 타임라인을 사용합니다.
- 스마트 엔진은 편집 단계에서 지연 적재하고 원본 교체 시 진행 중 추적 작업을 취소합니다.
- 실제 30분 1080p 영상에서 공간 모션·추적선·자막 안전 crop을 새로 검증했습니다.

# v1.6.4

- 저장소·오프라인 문제로 최하단 안내 후 자동 해결이 성공하면 최상단 작업 흐름으로 복귀합니다.
- 취소·실패·미해결 상태에서는 하단 위치를 유지합니다.
- 저장소 정리 전에 오래된 백업 수, 예상 로컬 용량, 이전 오프라인 캐시 수를 미리 보여줍니다.
- 정리 미리보기는 읽기 전용이며 현재 프로젝트와 최신 복구 데이터를 보존합니다.
- 저장소 전용 데스크톱·모바일 감사와 전체 224개 회귀를 통과했습니다.

# CHANGELOG

## v1.6.3 - Stage Focus & Progressive Disclosure

- 데스크톱 균형 작업실을 현재 단계 중심의 점진 공개 레이아웃으로 전환했습니다.
- 현재 작업은 확장하고 바로 이어지는 단계만 86px 지원 카드로 유지합니다.
- 지원 카드 클릭으로 다음 단계를 승격하고, 전체 보기로 모든 패널과 Local AI·프로젝트 도구를 복원합니다.
- 미리보기·파형 전용 모드와 모바일 단일 단계 흐름은 기존 동작을 유지합니다.
- 실제 하단 메뉴 높이를 측정해 콘텐츠 안전 여백을 자동 적용합니다.
- 직접 실행 스크립트 49개, CSS `!important` 593개, 충돌·중복·shadow 0을 유지했습니다.
- 자동 검사 224개와 4개 viewport·단계 집중·서비스워커·GPU/media·메모리 감사를 통과했습니다.
...
- 영구 분석 캐시 목록을 다중 선택 UI로 확장하고 비식별 토큰 기준 일괄 삭제, 계약·앱 버전 namespace의 안전한 정리, 삭제·무효화 통계를 추가했습니다.
- 서비스워커 무결성 감사에서 실패한 자산만 백오프를 우회해 즉시 재검증·복구하는 명령과 감사 이력·백오프 상태 초기화 명령을 추가했습니다.
- 서비스워커 등록 계층은 요청별 ID와 제한 시간으로 재시도·초기화 결과를 정확히 연결하며, 저장소 진단 화면에서 두 작업을 직접 실행할 수 있습니다.
- 중요 세션 백업을 독립 JSON 파일로 내보내고 다시 가져올 수 있으며, 가져오기 전에 형식·스키마·크기·체크섬을 검증합니다.
- 중요 백업에 최대 60자 이름과 240자 메모를 저장하고 복원 미리보기·진단 상태에 함께 표시합니다.
- 신규 회귀 검사 3개를 추가해 자동 QA **211/211**, Chromium 4개 viewport 오류·Promise 거절·콘솔 오류·가로 overflow **0건**을 통과했습니다.
---

## v1.5.26 - Adaptive Cache Quota, Integrity Audit Backoff & Protected Recovery

- IndexedDB 영구 분석 캐시가 `navigator.storage.estimate()` 결과에 따라 정상 8개·16MiB, 경고 60%, 위험 최소 2개·4MiB로 저장 한도를 자동 조정합니다.
- quota 오류 시 만료·LRU 항목을 정리한 뒤 쓰기를 한 번 재시도하고, 비식별 항목별 선택 삭제 기능을 추가했습니다.
- 서비스워커 감사 이력을 최대 40개 보관하고 반복 실패 자산에 5분~6시간 지수 백오프를 적용하며 온라인 복귀 후 검사를 재개합니다.
- 회전 정리되지 않는 중요 세션 백업과 복원 전 프로젝트 요약 미리보기를 추가했습니다.
- 신규 회귀 검사 4개를 포함해 자동 QA **208/208**을 통과했습니다.

---

## v1.5.25 - Persistent Analysis Cache, Selective Recovery & Rotating Integrity Audit

- 메모리 분석 캐시 뒤에 선택적 IndexedDB 영구 캐시를 추가했습니다. 앱 버전 namespace, 7일 TTL, 최대 8개 항목, 총 16MiB 제한과 LRU 정리를 적용합니다.
- 분석 캐시 적중·미적중·만료·퇴출·정리·지문 처리 비용을 개인정보 비노출 이벤트로 기록하고 JSON 진단 파일로 내보낼 수 있습니다.
- 진단 토큰은 원시 캐시 키 대신 FNV-1a 비식별 토큰을 사용하며 파일명·파일 경로·원본 미디어 내용은 포함하지 않습니다.
- 세션 패널에서 기본 기록과 각 순환 백업의 시점·압축률·검증 상태를 확인하고 원하는 정상 백업을 직접 선택해 복원할 수 있습니다.
- 손상된 백업은 목록에 표시하되 선택할 수 없도록 차단합니다.
- 서비스워커는 유휴 시간마다 12개 앱 셸 파일을 순환 SHA-256 검사하고 문제가 발견된 표본만 재다운로드·재검증합니다.
- 페이지 숨김·오프라인 상태에서는 자동 검사를 연기하며 수동 표본 검사와 검사 상태를 저장소 진단 화면에 추가했습니다.
- 영구 캐시·진단 내보내기·선택 복원·주기 무결성 검사 전용 회귀 검사 4개를 추가해 자동 QA **204/204**를 통과했습니다.
