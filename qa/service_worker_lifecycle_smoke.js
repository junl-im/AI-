...
ok(pkg.version === '1.6.5', 'service worker lifecycle release version is v1.6.5');
...
const reportPath = path.join(root, 'qa', 'runtime-service-worker-lifecycle-v1.6.5.json');
ok(fs.existsSync(reportPath), 'isolated lifecycle audit artifact exists');
const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
ok(report.version === '1.6.5' && report.install.shellCached && report.install.skipWaitingCalls === 1, 'audit confirms shell installation and skipWaiting');
...
console.log('PASS v1.6.5 observable service worker lifecycle guardrails');
