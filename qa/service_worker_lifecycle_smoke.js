...
ok(pkg.version === '1.5.20', 'service worker lifecycle release version is v1.5.20');
...
const reportPath = path.join(root, 'qa', 'runtime-service-worker-lifecycle-v1.5.20.json');
ok(fs.existsSync(reportPath), 'isolated lifecycle audit artifact exists');
const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
ok(report.version === '1.5.20' && report.install.shellCached && report.install.skipWaitingCalls === 1, 'audit confirms shell installation and skipWaiting');
...
console.log('PASS v1.5.20 observable service worker lifecycle guardrails');
