...
const projectService = {
    ...
    parseProjectText(text) { const parsed = JSON.parse(text); if (parsed.app !== 'AI Shorts Studio') throw new Error('bad app'); parsed.schemaVersion = 5; parsed.recommendations = parsed.recommendations || []; parsed.captions = parsed.captions || []; return parsed; },
    createProjectSnapshot(current) { return { app: 'AI Shorts Studio', schemaVersion: 5, fileMeta: current.fileMeta, fileName: current.fileMeta.name, recommendations: current.recommendations, selectedRecommendationId: current.selectedRecommendationId, selectedRange: current.selectedRange, captions: [], settings: {}, copy: {} }; }
};
const diagnostics = [];
const window = {
    window: null, document, localStorage, navigator: {}, caches: { async keys() { return []; } }, CustomEvent: function CustomEvent() {},
    AIShortsRuntimeConfig: { APP_VERSION: 'v1.6.13', BUILD_KEY: '1.6.13-transactional-model-pack-qa-controls', SESSION_SCHEMA_VERSION: 5, SESSION_BACKUP_COUNT: 2, SESSION_BACKUP_MAX_CHARS: 750000 },
...
if (!migrated || migrated.schemaVersion !== 5 || migrated.session.sourceSchemaVersion !== 3) throw new Error('v3 session must migrate to schema v5');
if (JSON.parse(values.get(primary)).schemaVersion !== 5) throw new Error('migrated session must repair the primary record');
...
console.log('PASS session schema v5 migration, rotating backup, and damaged-primary recovery');
