#!/usr/bin/env python3
"""Optional real-media Chromium audit for AI Shorts Studio v1.5.14.
...
OUTPUT = ROOT / 'qa' / 'runtime-media-e2e-v1.5.14.json'
...
def parse_args():
    ...
    parser.add_argument('--reset', action='store_true', help='Discard the existing v1.5.14 audit before running.')
...
def load_report(reset=False):
    if not reset and OUTPUT.exists():
        try:
            ...
            if existing.get('version') == '1.5.14':
                return existing
