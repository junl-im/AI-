...
const ids = ['visionModelPackPanel', 'visionPackStatus', 'visionPackDetail', 'visionPackSelect', 'visionPackBackend', 'visionPackInstallBtn', 'visionPackFolderInput', 'visionPackActivateBtn', 'visionPackDeactivateBtn', 'visionPackVerifyBtn', 'visionPackBenchmarkBtn', 'visionPackRollbackBtn', 'visionPackRecommendation', 'visionPackBenchmarkDetail', 'visionPackRemoveBtn', 'visionPackProgress', 'visionPackStorageSummary', 'visionPackStorageDetail', 'visionPackCleanupBtn'];
...
assert(panel.includes('installFromFiles') && panel.includes('verifyPack') && panel.includes('activatePack') && panel.includes('benchmarkPack') && panel.includes('rollbackToPrevious') && panel.includes('removePack') && panel.includes('cleanupOrphanedCache') && panel.includes('storageDiagnostics'), 'panel owns install, verify, benchmark, rollback, activation, removal, and orphan-cache cleanup actions');
assert(css.includes('.vision-model-pack-controls') && css.includes('.vision-model-pack-diagnostics') && css.includes('.vision-model-pack-storage') && css.includes('@media (max-width: 760px)'), 'vision model-pack controls and diagnostics include responsive ownership');
