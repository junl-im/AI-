...
assert(html.includes('assets/css/flow-quality-gate.css?v=1.5.24-compressed-session-integrity-rollback'), 'flow quality gate stylesheet is linked');
assert(loader.includes("versioned('src/ui/flow-quality-gate.js', 'shell')"), 'flow quality gate script is staged');
assert(sw.includes('./assets/css/flow-quality-gate.css?v=1.5.24-compressed-session-integrity-rollback'), 'flow quality gate stylesheet is cached');
...
assert(html.includes('data-build="1.5.24"'), 'build marker is v1.5.24');
