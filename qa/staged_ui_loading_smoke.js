...
assert(pkg.version === '1.2.6', 'staged hydration release version is v1.2.6');
assert(html.includes('src/boot/staged-ui-loader.js?v=1.2.6-staged-hydration'), 'staged UI loader is on the critical path');
assert(directScripts <= 39, `direct startup scripts are reduced to ${directScripts}`);
...
assert(sw.includes('staged-ui-loader.js?v=1.2.6-staged-hydration'), 'service worker caches the staged loader');
...
console.log('PASS v1.2.6 staged UI hydration guardrails present');
