...
const sw = read('sw.js');
assert(pkg.version === '1.5.25', 'header metadata release version is v1.5.25');
assert(!html.includes('LOCAL · PRIVATE · 9:16') && !html.includes('brand-compat-pill'), 'center metadata slogan and markup are absent');
assert(html.includes('>v1.5.25</button>') && html.includes('모바일 · PC 호환'), 'build version and compatibility share the left rail');
...
assert(html.includes('header-meta-rail.css?v=1.5.25-persistent-analysis-selective-recovery-integrity-audit') && sw.includes('header-meta-rail.css?v=1.5.25-persistent-analysis-selective-recovery-integrity-audit'), 'metadata rail stylesheet is loaded and cached');
console.log('PASS v1.5.25 header metadata rail guardrails present');
