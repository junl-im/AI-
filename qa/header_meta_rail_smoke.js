...
const sw = read('sw.js');
assert(pkg.version === '1.6.3', 'header metadata release version is v1.6.3');
assert(!html.includes('LOCAL · PRIVATE · 9:16') && !html.includes('brand-compat-pill'), 'center metadata slogan and markup are absent');
assert(html.includes('>v1.6.3</button>') && html.includes('모바일 · PC 호환'), 'build version and compatibility share the left rail');
...
assert(html.includes('header-meta-rail.css?v=1.6.3-stage-focus-progressive-disclosure') && sw.includes('header-meta-rail.css?v=1.6.3-stage-focus-progressive-disclosure'), 'metadata rail stylesheet is loaded and cached');
console.log('PASS v1.6.3 header metadata rail guardrails present');
