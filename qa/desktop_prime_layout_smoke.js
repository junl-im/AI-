...
assert(pkg.version === '1.1.9', 'desktop prime release version is v1.1.9');
assert(html.includes('assets/css/desktop-prime-layout.css?v=1.1.9-desktop-prime'), 'desktop prime stylesheet is linked last');
assert(sw.includes('desktop-prime-layout.css?v=1.1.9-desktop-prime'), 'desktop prime stylesheet is cached');
...
assert(director.includes('isDesktopPrime()') && director.includes("panel.setAttribute(\'aria-hidden\', prime || match"), 'flow director keeps prime panels accessible');
...
assert(startPanel.includes('flow-overview-copy') && startPanel.includes('Dock에서 파일을 열고'), 'mobile landing is a flow overview');
...
console.log('PASS v1.1.9 desktop prime and mobile flow-only guardrails present');
