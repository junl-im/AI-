...
const previewJs = fs.readFileSync(path.join(root, 'src/app/preview-controller.js'), 'utf8');
...
if (!appJs.includes('getPreviewController') || !previewJs.includes('stillRaf') || !previewJs.includes('requestFrame(renderStillNow)')) {
    console.error('FAIL preview still rendering is not isolated and RAF-batched');
    process.exit(1);
}
