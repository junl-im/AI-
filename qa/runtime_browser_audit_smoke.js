...
const auditPath = path.join(root, 'qa', 'runtime-browser-audit-v1.5.4.json');
...
assert(fs.existsSync(auditPath), 'v1.5.4 browser audit artifact exists');
const report = JSON.parse(fs.readFileSync(auditPath, 'utf8'));
assert(report.version === '1.5.4', 'browser audit version matches release');
...
console.log('PASS v1.5.4 real-browser stability, adaptive mobile menu, stage landing and workspace control audit');
