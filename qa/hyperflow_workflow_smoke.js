...
if (!html.includes('원본을 불러오면 자동 분석') || !html.includes('data-icon="spark"') || !html.includes('>추천 생성</button>')) {
...
if (!app.includes('hasAnalysisReady') || !app.includes("els.analyzeBtn.textContent = state.isAnalyzing ? '자동 분석 중' : '추천 생성'") || !app.includes("els.analyzeBtn.dataset.icon = state.isAnalyzing ? 'render' : 'spark'")) {
