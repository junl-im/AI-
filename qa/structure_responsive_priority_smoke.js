...
const reduction = readJson('qa/runtime-structure-priority-v1.5.20.json');
const probe = readJson('qa/runtime-structure-priority-probe-v1.5.20.json');
...
ok(pkg.version === '1.5.20', 'structure and responsive priority release version is v1.5.20');
ok(reduction.baselineVersion === '1.5.19' && reduction.targetVersion === '1.5.20', 'reduction is anchored to the v1.5.19 baseline');
...
ok(probe.safeCount >= 200 && probe.unsafeCount > 0 && probe.errors.length === 0, 'Chromium probe distinguishes safe and required structural priorities without runtime errors');
...
console.log('PASS v1.5.20 structural and responsive priority reduction with unchanged layout metrics');
