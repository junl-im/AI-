...
(async () => {
    ...
    const currentName = [...h.stores.keys()].find(name => name.includes('v1.5.25'));
    const store = h.stores.get(currentName);
    if (!store) throw new Error('current integrity cache missing');
    const target = './assets/css/theme.css?v=1.5.25-persistent-analysis-selective-recovery-integrity-audit';
