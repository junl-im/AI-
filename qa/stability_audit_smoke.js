...
assert(pkg.version === '1.2.9' && pkg.scripts.test === 'npm run check', 'release version and standard test alias are aligned');
assert(loader.includes("config.APP_VERSION || 'v1.2.9'") && loader.includes('config.BUILD_KEY'), 'staged modules inherit the current runtime version and build key');
...
console.log('PASS v1.2.9 code and engine stability guardrails present');
