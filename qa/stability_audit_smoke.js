...
const preview = read('src/app/preview-controller.js');
const session = read('src/ui/session-continuity.js');
const bridge = read('src/ui/flow-command-bridge.js');

assert(pkg.version === '1.6.15' && pkg.scripts.test === 'npm run check', 'release version and standard test alias are aligned');
assert(loader.includes("config.APP_VERSION || 'v1.6.15'") && loader.includes('config.BUILD_KEY'), 'staged modules inherit the current runtime version and build key');
...
console.log('PASS v1.6.15 code and engine stability guardrails present');
