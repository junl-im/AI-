...
assert(pkg.version === '1.3.2' && pkg.scripts.test === 'npm run check', 'release version and standard test alias are aligned');
assert(loader.includes("config.APP_VERSION || 'v1.3.2'") && loader.includes('config.BUILD_KEY'), 'staged modules inherit the current runtime version and build key');
...
console.log('PASS v1.3.2 code and engine stability guardrails present');
