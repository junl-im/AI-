...
assert(html.includes('src/engine/operation-coordinator.js?v=1.5.0-experience-engine'), 'operation coordinator is loaded');
...
assert(app.includes("startMediaSession({ fileName: file.name") && app.includes("beginOperation('analysis'") && app.includes("beginOperation('preview'") && workflow.includes("beginOperation('render'"), 'app and render controller coordinate media, analysis, preview and render generations');
assert(app.includes('renderQueue.cancel') && workflow.includes('assertOperation(token'), 'file replacement cancels render and stale results are rejected');
