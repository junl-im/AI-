...
ok(/^1\.6\.\d+$/.test(pkg.version), 'workflow focus release version is v1.6.20');
...
ok(html.includes(`assets/css/workflow-focus-layout.css?v=${buildKey}`), 'workflow focus stylesheet is loaded last in layout ownership');
...
ok(sw.includes(`./assets/css/workflow-focus-layout.css?v=${buildKey}`) && sw.includes(`./src/ui/workflow-focus-layout.js?v=${buildKey}`), 'service worker caches stage focus assets');
...
console.log('PASS v1.6.25 stage-aware progressive disclosure, reversible full view, and measured dock clearance');
