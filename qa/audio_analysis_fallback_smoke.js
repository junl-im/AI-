...
assert(/^1\.6\.\d+$/.test(pkg.version), 'audio fallback release version is v1.6.20');
assert(html.includes(`audio-analysis-core.js?v=${buildKey}`) && html.indexOf('audio-analysis-core.js') < html.indexOf('audio-feature-extractor.js'), 'shared audio core loads before the extractor');
assert(sw.includes(`audio-analysis-core.js?v=${buildKey}`), 'service worker caches the shared audio core');
...
console.log('PASS v1.6.25 shared audio analyzer and worker fallback guardrails');
