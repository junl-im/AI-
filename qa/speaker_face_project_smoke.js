...
(function () {
  ...
  const state = {
    ...
    smartReframeEdits: {
      subjectId: 'auto', keyframes: [], speakerPriority: true, speakerLayout: { split: .61, primaryPosition: 'bottom', gridPrimarySize: .6, gridPrimaryPosition: 'right', gridPaging: 'manual', gridPageSeconds: 2.5, gridEnergyThreshold: .42, gridEnergyHysteresis: .11, gridEnergyHoldSeconds: 1.7, gridTransition: 'slide', gridTransitionMs: 480, gridTransitionEasing: 'ease-out', gridSlideDirection: 'down', gridManualPages: [['subject-2','subject-1','subject-3']], gridManualPageSeconds: [4.5] },
  ...
  ok(target.smartReframeEdits.speakerPriority === true && target.smartReframeEdits.speakerLayout.split === .61 && target.smartReframeEdits.speakerLayout.primaryPosition === 'bottom' && target.smartReframeEdits.speakerLayout.gridPrimaryPosition === 'right' && target.smartReframeEdits.speakerLayout.gridPaging === 'manual' && target.smartReframeEdits.speakerLayout.gridEnergyThreshold === .42 && target.smartReframeEdits.speakerLayout.gridTransitionEasing === 'ease-out' && target.smartReframeEdits.speakerLayout.gridSlideDirection === 'down' && target.smartReframeEdits.speakerLayout.gridManualPages[0][1] === 'subject-1' && target.smartReframeEdits.speakerLayout.gridManualPageSeconds[0] === 4.5 && target.smartReframeEdits.speakerCues[0].gridCrop.y === -.08 && target.smartReframeEdits.speakerCues[0].subjectId === 'subject-2' && target.smartReframeEdits.speakerCues[0].locked === true, 'speaker direction and pane layout restore on project import');
