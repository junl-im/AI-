...
assert(pkg.version === '1.5.25', 'UI refinement release version is v1.5.25');
assert(html.includes('assets/css/ui-refinement.css?v=1.5.25-persistent-analysis-selective-recovery-integrity-audit'), 'UI refinement stylesheet is loaded last');
assert(html.indexOf('ui-refinement.css') > html.indexOf('hero-command-deck.css'), 'UI refinement stylesheet wins the cascade');
assert(sw.includes('./assets/css/ui-refinement.css?v=1.5.25-persistent-analysis-selective-recovery-integrity-audit'), 'service worker caches the UI refinement stylesheet');
...
console.log('PASS v1.5.25 unified UI refinement guardrails present');
