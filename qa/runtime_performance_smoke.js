...
assert(pkg.version === '1.6.13', 'runtime polish package version is v1.6.13');
assert(html.includes('assets/css/foundation-polish.css?v=1.6.13-transactional-model-pack-qa-controls'), 'foundation polish stylesheet is linked');
assert(html.includes('src/ui/startup-performance.js?v=1.6.13-transactional-model-pack-qa-controls'), 'startup performance module is linked');
assert(sw.includes('foundation-polish.css?v=1.6.13-transactional-model-pack-qa-controls'), 'foundation polish stylesheet is cached');
assert(sw.includes('startup-performance.js?v=1.6.13-transactional-model-pack-qa-controls'), 'startup performance module is cached');
assert(!ux.includes('setInterval(uxSyncAll, 700)'), '700ms UX polling loop is removed');
assert(ux.includes('MutationObserver(uxScheduleSync)') && (ux.includes('queueMicrotask') || ux.includes('requestAnimationFrame')), 'UX synchronization is event-driven and coalesced without polling');
...
console.log('PASS v1.6.13 runtime performance and startup hierarchy guardrails present');
