...
assert(pkg.version === '1.0.2', 'package version must be 1.0.2');
assert(html.includes('assets/css/flow-doctor.css'), 'flow doctor css must be linked');
assert(html.includes('src/ui/flow-doctor.js'), 'flow doctor script must be loaded');
...
assert(html.includes('v1.0.2</button>'), 'header version must be simple v1.0.2');
assert(html.includes('Design by <strong>곰같은여우</strong>'), 'designer signature must remain in header');
assert(tabs.includes('setActiveFlowTab(key, { reveal: false })'), 'manual dock tab clicks must not force reveal/scroll');
