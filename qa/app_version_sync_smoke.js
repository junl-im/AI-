...
assert(config.includes(`BUILD_KEY: '${plain}-stage-beacon'`), 'runtime config build key is updated');
...
assert(sw.includes(`ai-shorts-studio-shell-${version}-stage-beacon`), 'service worker cache name matches the current build');
assert(sw.includes('networkFirst') && sw.includes('request.mode === \'navigate\''), 'service worker uses network-first navigation shell');
assert(sw.includes(`app-version-sync.js?v=${plain}-stage-beacon`), 'service worker precaches version sync module');
