...
assert(config.includes(`BUILD_KEY: '${plain}-header-meta'`), 'runtime config build key is updated');
...
assert(sw.includes(`ai-shorts-studio-shell-${version}-header-meta`), 'service worker cache name matches the runtime stability build');
assert(sw.includes('networkFirst') && sw.includes('request.mode === \'navigate\''), 'service worker uses network-first navigation shell');
assert(sw.includes(`app-version-sync.js?v=${plain}-header-meta`), 'service worker precaches version sync module');
