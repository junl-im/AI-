...
assert(config.includes(`BUILD_KEY: '${plain}-css-ownership'`), 'runtime config build key is updated');
...
assert(versionedAssets.length > 40 && versionedAssets.every(value => value === `${plain}-css-ownership`), 'all core HTML assets use the current build key');
...
assert(sw.includes(`ai-shorts-studio-shell-${version}-css-ownership`), 'service worker cache name matches the current build');
assert(sw.includes('networkFirst') && sw.includes('request.mode === \'navigate\''), 'service worker uses network-first navigation shell');
assert(sw.includes(`app-version-sync.js?v=${plain}-css-ownership`), 'service worker precaches version sync module');
assert(html.includes('src/boot/service-worker-registration.js'), 'service worker registration module is loaded');
assert(sw.includes(`service-worker-registration.js?v=${plain}-css-ownership`), 'service worker precaches registration module');
