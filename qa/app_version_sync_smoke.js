...
assert(config.includes(`BUILD_KEY: '${plain}-workspace-control'`), 'runtime config build key is updated');
...
assert(sw.includes(`ai-shorts-studio-shell-${version}-workspace-control`), 'service worker cache name matches the current build');
assert(sw.includes('networkFirst') && sw.includes('request.mode === \'navigate\''), 'service worker uses network-first navigation shell');
assert(sw.includes(`app-version-sync.js?v=${plain}-workspace-control`), 'service worker precaches version sync module');
