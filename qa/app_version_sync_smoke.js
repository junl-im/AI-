...
assert(config.includes(`BUILD_KEY: '${plain}-stability-audit'`), 'runtime config build key is updated');
...
assert(sw.includes(`ai-shorts-studio-shell-${version}-stability-audit`), 'service worker cache name matches the runtime stability build');
assert(sw.includes('networkFirst') && sw.includes('request.mode === \'navigate\''), 'service worker uses network-first navigation shell');
assert(sw.includes(`app-version-sync.js?v=${plain}-stability-audit`), 'service worker precaches version sync module');
