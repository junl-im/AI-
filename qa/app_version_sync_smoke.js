...
assert(config.includes(`BUILD_KEY: '${plain}-experience-engine'`), 'runtime config build key is updated');
...
assert(versionedAssets.length > 40 && versionedAssets.every(value => value === `${plain}-experience-engine`), 'all core HTML assets use the current build key');
assert(!html.includes('1.3.4-experience-engine') && !html.includes('1.3.5-experience-engine'), 'stale HTML build keys are absent');
...
assert(sw.includes(`ai-shorts-studio-shell-${version}-experience-engine`), 'service worker cache name matches the current build');
assert(sw.includes('networkFirst') && sw.includes('request.mode === \'navigate\''), 'service worker uses network-first navigation shell');
assert(sw.includes(`app-version-sync.js?v=${plain}-experience-engine`), 'service worker precaches version sync module');
assert(html.includes('src/boot/service-worker-registration.js'), 'service worker registration module is loaded');
assert(sw.includes(`service-worker-registration.js?v=${plain}-experience-engine`), 'service worker precaches registration module');
