...
ok(html.includes('assets/css/storage-health-panel.css?v=1.6.0-local-ai-provider-foundation'), 'storage health stylesheet is linked');
ok(html.includes('src/storage/storage-manager.js?v=1.6.0-local-ai-provider-foundation'), 'quota-aware storage manager is on the core path');
ok(loader.includes("versioned('src/ui/storage-health-panel.js', 'shell')"), 'storage health UI is loaded with the shell phase');
ok(sw.includes('storage-health-panel.css?v=1.6.0-local-ai-provider-foundation') && sw.includes('storage-manager.js?v=1.6.0-local-ai-provider-foundation'), 'storage diagnostics assets are available offline');
...
console.log('PASS v1.6.0 storage, cache signature, trend, and offline diagnostics panel guardrails');
