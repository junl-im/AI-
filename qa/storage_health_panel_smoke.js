...
ok(html.includes('assets/css/storage-health-panel.css?v=1.5.24-compressed-session-integrity-rollback'), 'storage health stylesheet is linked');
ok(html.includes('src/storage/storage-manager.js?v=1.5.24-compressed-session-integrity-rollback'), 'quota-aware storage manager is on the core path');
ok(loader.includes("versioned('src/ui/storage-health-panel.js', 'shell')"), 'storage health UI is loaded with the shell phase');
ok(sw.includes('storage-health-panel.css?v=1.5.24-compressed-session-integrity-rollback') && sw.includes('storage-manager.js?v=1.5.24-compressed-session-integrity-rollback'), 'storage diagnostics assets are available offline');
...
console.log('PASS v1.5.24 storage and offline diagnostics panel guardrails');
