...
ok(html.includes('assets/css/storage-health-panel.css?v=1.6.3-stage-focus-progressive-disclosure'), 'storage health stylesheet is linked');
ok(html.includes('src/storage/storage-manager.js?v=1.6.3-stage-focus-progressive-disclosure'), 'quota-aware storage manager is on the core path');
ok(loader.includes("versioned('src/ui/storage-health-panel.js', 'shell')"), 'storage health UI is loaded with the shell phase');
ok(sw.includes('storage-health-panel.css?v=1.6.3-stage-focus-progressive-disclosure') && sw.includes('storage-manager.js?v=1.6.3-stage-focus-progressive-disclosure'), 'storage diagnostics assets are available offline');
...
ok(panel.includes('mountAtPageEnd') && panel.includes('navigateToHealthPanel'), 'storage status is footer-mounted with issue-driven navigation');
...
console.log('PASS v1.6.3 storage, cache signature, trend, and offline diagnostics panel guardrails');
