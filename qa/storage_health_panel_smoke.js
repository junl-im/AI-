...
ok(html.includes('assets/css/storage-health-panel.css?v=1.5.27-selective-cache-integrity-retry-portable-backup'), 'storage health stylesheet is linked');
ok(html.includes('src/storage/storage-manager.js?v=1.5.27-selective-cache-integrity-retry-portable-backup'), 'quota-aware storage manager is on the core path');
ok(loader.includes("versioned('src/ui/storage-health-panel.js', 'shell')"), 'storage health UI is loaded with the shell phase');
ok(sw.includes('storage-health-panel.css?v=1.5.27-selective-cache-integrity-retry-portable-backup') && sw.includes('storage-manager.js?v=1.5.27-selective-cache-integrity-retry-portable-backup'), 'storage diagnostics assets are available offline');
...
console.log('PASS v1.5.27 storage and offline diagnostics panel guardrails');
