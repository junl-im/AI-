#!/usr/bin/env python3
"""Chromium audit for stable energy paging, manual page ordering, transition controls, and bulk grid crops."""
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.evaluate("""() => {
            ...
            const cues = subjects.map((subject, index) => ({ start: index === 4 ? 1.5 : 0, end: 8, speaker: subject.label, subjectId: subject.id, confidence: .9 - index * .02, energy: energy[index], priority: index === 0 ? 'primary' : 'auto', gridCrop: {} }));
            const track = AIShortsSmartReframe._test.buildTrack([point(0,.5),point(8,.5)], 'hybrid', {}, {}, { subjects, activeSubjectId: 'auto', speakerPriority: true, speakerCues: cues, speakerLayout: { gridPaging: 'energy', gridEnergyThreshold: .5, gridEnergyHysteresis: .08, gridEnergyHoldSeconds: 1.2, gridTransition: 'fade' } });
        ...
        energy = await page.evaluate("""() => {
            const early = AIShortsSmartReframe.getFocusAt(AIShortsAppState.state.smartReframe, 2);
            const settled = AIShortsSmartReframe.getFocusAt(AIShortsAppState.state.smartReframe, 3);
            const layout = AIShortsAppState.state.smartReframe.speakerLayout;
            return { early: early.gridSubjects.map(item => item.subjectId), settled: settled.gridSubjects.map(item => item.subjectId), trigger: settled.gridPageTrigger, threshold: layout.gridEnergyThreshold, hysteresis: layout.gridEnergyHysteresis, hold: layout.gridEnergyHoldSeconds };
        }""")
        await page.evaluate("""() => {
            ...
            change('#speakerGridTransitionEasingSelect', 'ease-out');
            change('#speakerGridSlideDirectionSelect', 'down');
        }""")
        ...
        manual = await page.evaluate("""() => {
            ...
            return {
                ...
                easing: boundary.speakerLayout.gridTransitionEasing,
                direction: boundary.speakerLayout.gridSlideDirection,
                pagesText: document.querySelector('#speakerGridManualPagesInput').value
            };
        }""")
        reorder = await page.evaluate("""() => {
            const cards = Array.from(document.querySelectorAll('.speaker-grid-manual-page-card'));
            const before = cards.map(card => card.textContent.trim());
            const up = cards[1] && cards[1].querySelector('.speaker-grid-manual-page-actions button');
            if (up) up.click();
            return {
                before,
                after: Array.from(document.querySelectorAll('.speaker-grid-manual-page-card')).map(card => card.textContent.trim()),
                pages: AIShortsAppState.state.smartReframe.speakerLayout.gridManualPages,
                text: document.querySelector('#speakerGridManualPagesInput').value
            };
        }""")
    ...
    checks = {
        'energyStabilityControls': energy['trigger'] == 'energy' and energy['threshold'] == .5 and energy['hysteresis'] == .08 and energy['hold'] == 1.2 and energy['early'][1] == 'subject-2' and energy['settled'][1] == 'subject-5',
        'manualPageOrder': manual['previous'] == ['subject-1','subject-5','subject-3'] and manual['second'] == ['subject-1','subject-6','subject-2','subject-4'],
        'slideTransitionState': manual['transition'] == 'slide' and manual['easing'] == 'ease-out' and manual['direction'] == 'down' and manual['previous'] == ['subject-1','subject-5','subject-3'] and 0 < manual['progress'] < 1,
        'manualPagesPersistInUi': 'subject-1,subject-5,subject-3' in manual['pagesText'],
        'manualPageReorder': reorder['pages'][0] == ['subject-1','subject-6','subject-2','subject-4'] and reorder['pages'][1] == ['subject-1','subject-5','subject-3'],
