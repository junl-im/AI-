...
```text
fix/setup-uv-action-pin
```

권장 커밋 메시지:

```text
fix: pin setup-uv action to verified commit
```
...
- `setup-uv`가 `08807647e7069bb48b6ef5acd8ec9567f424441b # v8.1.0`으로 고정됐는지 확인합니다.
- uv 실행 파일 버전이 `0.11.32`로 고정됐는지 확인합니다.
...
- 상단에 `BUILD v0.5.4`이 표시되는지 확인합니다.
...
3. 공개 주소에서 `BUILD v0.5.4`을 확인합니다.
