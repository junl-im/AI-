...
# HANDOFF v1.5.29 — Option-Aware Analysis Cache & Storage Cost Trend

## 완료한 패치

- 자동 컷 설정이 분석 캐시 키에 빠져 설정 변경 뒤 stale `autoCuts`가 재사용될 수 있던 결함을 수정했습니다.
- 키 순서에 무관한 canonical serialization과 16자리 비식별 option signature를 추가했습니다.
- 분석 시작 시 옵션을 한 번만 스냅샷해 키 생성과 실제 파이프라인이 동일 값을 사용하도록 했습니다.
- 분석 캐시 계약을 v3로 올리고 기존 v2 자료는 이전 namespace로 보존했습니다.
- signature별 항목·바이트·마지막 사용 집계와 선택 정리를 저장소 진단 화면에 추가했습니다.
- namespace 저장 비용 추세를 최대 48개로 보존하고 현재 사용량과 직전 대비 증감을 표시합니다.
- 결합 유지보수 스냅샷으로 진단 새로고침의 IndexedDB 전체 스캔을 최대 3회에서 1회로 줄였습니다.

## 유지해야 할 계약

1. 분석 결과에 영향을 주는 옵션은 반드시 option signature 입력에 포함합니다.
2. 옵션은 분석 시작 시 한 번만 읽고, 그 스냅샷을 캐시 키와 실제 분석이 공유합니다.
3. 현재 분석 캐시 계약은 `v3`입니다. 이전 계약 namespace는 사용자 확인 전에 자동 삭제하지 않습니다.
4. signature와 namespace 표시는 원문이 아닌 비식별 토큰·요약 정보만 사용합니다.
5. 저장 비용 추세는 최대 48개이며 동일 상태를 중복 기록하지 않습니다.
6. 저장소 진단 UI는 정책 갱신 뒤 `maintenanceSnapshot({ refresh: false })`를 우선 사용해 추가 전체 스캔을 만들지 않습니다.
7. 현재 namespace와 알 수 없는 토큰은 선택 정리에서 안전하게 보호·무시합니다.
8. 앱 셸 파일을 변경하면 build key, 서비스워커 cache name, asset integrity manifest를 함께 갱신합니다.
...
- 자동 QA **213/213**
  ...
  - 172~190: 19개 통과
  - 191~212: 22개 통과
- 신규 signature/trend 회귀 통과
- Chromium desktop·small laptop·tablet·mobile 오류·거절·console error·horizontal overflow 0
- CSS conflict·same-value duplicate·shadowed declaration 0, `!important` 593
- 서비스워커 install·activate·offline navigation 통과
- GPU/media 두 모드 62프레임 decode, dropped frame 0

## 감사 자료 구분

신규 실행:
- `runtime-browser-audit-v1.5.29.json`
- `runtime-css-ownership-v1.5.29.json`
- `runtime-interaction-state-v1.5.29.json`
- `runtime-service-worker-lifecycle-v1.5.29.json`
- `runtime-gpu-media-capability-v1.5.29.json`

변경 범위 밖 상속:
- `runtime-process-memory-v1.5.29.json`
- `runtime-structure-priority-v1.5.29.json`
- `runtime-structure-priority-probe-v1.5.29.json`
- `runtime-long-video-stability-v1.5.29.json`

## 다음 우선순위

1. 서비스워커 실패 자산별 상세 원인·수동 롤백
2. 중요 백업 보존 개수·복구 이력 편집
3. 현재 namespace 저장 비용 예측·자동 경고 임계값
