# PROJECT NOTES v1.5.26

## 영구 분석 캐시 quota 규칙

- 영구 캐시는 브라우저 저장소 사용률이 80% 미만이면 기본 8개·16MiB를 사용합니다.
- 사용률 80% 이상 92% 미만은 기본 한도의 60%, 92% 이상은 최소 2개·4MiB로 축소합니다.
- quota 정책은 최대 60초 간격으로 다시 계산하며 변경 시 진단 이벤트를 기록합니다.
- quota 쓰기 오류는 만료·LRU 정리를 수행한 뒤 한 번만 재시도하고 분석 실행 자체는 실패시키지 않습니다.
- 선택 삭제 식별자는 원시 캐시 키가 아닌 FNV-1a 비식별 토큰을 사용합니다.

## 서비스워커 감사 규칙

- 무결성 감사 이력은 최신 순 최대 40개를 설치 보고서에 보관합니다.
- 자산별 실패 백오프는 5분에서 시작해 지수 증가하며 최대 6시간입니다.
- 백오프 중인 자산은 표본에서 제외하고 정상 확인 또는 복구 성공 시 상태를 제거합니다.
- 온라인 복귀 시 1초 뒤 미뤄진 무결성 검사를 다시 예약합니다.
- 진단 내보내기는 감사·복구·백오프·롤백 상태만 포함하고 사용자 프로젝트 데이터는 포함하지 않습니다.

## 중요 세션 백업 규칙

- 중요 백업은 `${STORAGE_KEY}-protected` 단일 슬롯이며 순환 백업 정리 대상이 아닙니다.
- 중요 백업은 검증 가능한 정상 스냅샷만 생성할 수 있고 자동 복구에서 순환 백업보다 우선합니다.
- 복원 미리보기에는 프로젝트 제목, 추천 후보 수, 자막 수, 선택 구간, 압축 상태만 표시합니다.
- 사용자의 명시적 보호 해제 또는 세션 기록 전체 삭제는 중요 백업을 제거할 수 있습니다.

## QA·배포 기준

- 자동 QA 기준은 **208/208**입니다.
...
- 런타임 build key는 `1.5.26-adaptive-cache-audit-protected-recovery`입니다.
...
1. 분석 옵션·엔진 계약별 캐시 무효화와 이전 namespace 정리
2. 다중 항목 삭제·quota 추세·저장 비용 상세 진단
3. 서비스워커 감사 이력 관리와 실패 자산 즉시 재시도 UI
4. 중요 백업 내보내기·가져오기와 보존 정책 설정
5. 실제 모바일 브라우저와 물리 GPU 장시간 검증
