# PROJECT NOTES v1.5.25

## 분석 캐시 계층 규칙

- 분석 조회 순서는 메모리 LRU 캐시 → 선택적 IndexedDB 영구 캐시 → 새 분석입니다.
- 영구 캐시 namespace는 엔진 버전에 묶이며 기본 제한은 7일 TTL, 최대 8개 항목, 총 16MiB입니다.
- IndexedDB 미지원·차단·quota 오류는 분석 실패로 전파하지 않고 메모리 캐시 또는 새 분석으로 폴백합니다.
- 영구 캐시 정리는 만료 항목을 먼저 제거한 뒤 마지막 사용 시각 기준 LRU로 항목 수·용량 제한을 만족시킵니다.

## 분석 진단 개인정보 규칙

- 진단 이벤트는 최대 80개를 순환 보관합니다.
- 파일명, 로컬 경로, 원본 미디어, 원시 캐시 키, 분석 결과 본문은 내보내지 않습니다.
- 캐시 항목 식별에는 원시 키의 FNV-1a 비식별 토큰만 사용합니다.
- 내보내기에는 메모리·영구 캐시 집계, 적중·미적중·만료·퇴출·정리 횟수, 지문 처리 시간과 읽은 바이트만 포함합니다.

## 선택 세션 복원 규칙

- 복원 목록은 기본 세션과 모든 순환 백업을 각각 디코딩·체크섬·프로젝트 스키마 검증한 뒤 구성합니다.
- 정상 스냅샷만 선택 가능하며 손상 백업은 상태를 표시하되 복원 버튼 대상에서 제외합니다.
- 사용자가 선택한 source key를 그대로 복원하고 성공·실패를 기존 복구 이력에 기록합니다.

## 서비스워커 순환 무결성 규칙

- 자동 검사는 초기 30초 뒤 시작하며 이후 기본 15분 간격으로 실행합니다.
- 한 주기마다 12개 앱 셸 파일을 검사하고 cursor를 저장해 다음 주기에 이어서 순환합니다.
- 페이지가 숨겨졌거나 오프라인이면 검사를 연기하고, 가능한 환경에서는 idle callback 안에서 실행합니다.
- 누락·HTTP 비정상·SHA-256 불일치가 확인된 표본만 기존 복구 경로로 재다운로드·재검증합니다.

## QA·배포 기준

- 자동 QA 기준은 **204/204**입니다.
...
- 런타임 build key는 `1.5.25-persistent-analysis-selective-recovery-integrity-audit`입니다.
...
1. quota pressure 기반 영구 분석 캐시 한도 자동 조정과 저장 항목별 비용 진단
2. 서비스워커 주기 감사 이력·복구 백오프·진단 JSON
3. 백업 보존 개수 설정·복원 미리보기·이력 관리 UI
