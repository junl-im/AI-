// AI Shorts Studio v1.3.1 - cancellable video motion analyzer
'use strict';
...
(function exposeVideoMotionAnalyzer(global) {
    ...
    async function seekVideo(video, time, signal) {
        ...
        const promise = waitForEvent(video, 'seeked', 2200, signal).catch(error => {
    ...
    async function analyzeVideoMotion(fileUrl, onProgress, signal) {
        throwIfAborted(signal);
