...
function reviewId(sentence: string, engineId: string): string {
  let hash = 2166136261
  const value = `${engineId}\u0000${sentence}`
  for (let index = 0; index < value.length; index += 1) {
    hash ^= value.charCodeAt(index)
    hash = Math.imul(hash, 16777619)
  }
  return `${engineId}-${(hash >>> 0).toString(16)}`
}

export async function saveQualityReview(input: QualityReviewInput): Promise<QualityReview> {
  const id = reviewId(input.sentence, input.engineId)
  const existing = await getQualityReview(input.sentence, input.engineId)
...
export async function getQualityReview(sentence: string, engineId: string): Promise<QualityReview | null> {
  const database = await openSoriOnDatabase()
  const review = await new Promise<QualityReview | null>((resolve, reject) => {
    const request = database.transaction(QUALITY_REVIEW_STORE, 'readonly')
      ...
      .get(reviewId(sentence, engineId))
