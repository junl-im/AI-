...
각 Worker 그룹은 시간순 최초 5건과 최근 5건을 비중첩 window로 비교합니다. 총 10건 미만은
`insufficient`로 표시하며 통과 또는 안정 상태로 간주하지 않습니다.

- 실패율이 기준보다 5%p 초과 증가
- first audio P95가 25% 이상이면서 100ms 이상 증가
- RTF P95가 20% 이상이면서 0.05 이상 증가
- final handoff P95가 50% 이상이면서 50ms 이상 증가

한 지표는 `warning`, 두 지표 이상 또는 최근 실패율 20% 이상은 `regressed`입니다. 이는 자동
경보이며 실기기 10·30·60분 인증과 운영자 판단을 대체하지 않습니다.
