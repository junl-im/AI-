...
| --- | --- | --- | --- | ---: | ---: | ---: | --- |
| `sori-warm` | 혜린 | 여성 | 따뜻한 내레이션·안내 | 1.00 | 0.5 | 여성 1번 | `sori-warm.wav` |
| `on-clear` | 도윤 | 남성 | 또렷한 설명·교육 | 1.04 | -0.5 | 남성 1번 | `on-clear.wav` |
| `dam-calm` | 소리 | 중성 | 차분한 장문·명상 | 0.98 | 0 | 중성 1번 | `dam-calm.wav` |
| `jun-deep` | 준호 | 남성 | 저음 다큐·오디오북 | 0.98 | -1.0 | 남성 2번 | `jun-deep.wav` |
| `min-energetic` | 민준 | 남성 | 활력 광고·숏폼 | 1.08 | 0.25 | 남성 3번 | `min-energetic.wav` |
...
- 혜린의 시스템 근사 기본 높낮이는 `+1.5 -> +0.5`로 낮춰 얇고 금속성으로 들리는 현상을 줄입니다.
- 도윤/소리/준호/민준도 시스템 근사 pitch offset을 각각 `-0.5 / 0 / -1.0 / +0.25`로 축소합니다.
- Browser Speech는 사용자 pitch를 40%만 반영하고, preset offset과 합친 값을 12음 평균율 비율로 변환한 뒤 Web Speech pitch를 `0.90~1.12`로 제한합니다.
- 프리셋 캐릭터는 과도한 pitch 변조보다 실제 한국어 voice identity, 속도, 그리고 검증된 neural reference를 우선합니다.
- Browser Speech는 PC/모바일 공통의 **기기 내장 근사 미리듣기**이며, CosyVoice 등 neural WAV와 같은 품질로 표시하지 않습니다.
- 카카오 WebView에서 Speech Synthesis가 시작되지 않는 경우 1.8초 watchdog과 외부 브라우저 복구 경로를 유지합니다. 이 fallback은 WebView 자체의 음성 지원을 강제로 바꾸는 기능은 아닙니다.
...
- 도윤·준호·민준에 필요한 남성 후보가 하나뿐이면 같은 성별 목소리를 프리셋별 속도·높낮이 설정과 함께 순환 사용합니다. 반대 성별은 사용하지 않습니다.
...
`auto` 생성에서 서버 엔진이 특정 프리셋만 표현하지 못하면 `SOA-4022`로 분리하고, 호환 가능한 Browser Speech가 있으면 마지막 경로로 이어서 시도합니다. System TTS는 Windows/macOS 백엔드 뒤에 설치된 eSpeak 한국어 백엔드를 보조 경로로 유지합니다. 이 동작은 **생성 실패를 줄이기 위한 로컬 근사 폴백**이며 전용 CosyVoice 인물 음색 보장은 아닙니다. 실제 동일 인물 음색은 계속 검증된 동일 ID WAV와 manifest가 필요합니다.
