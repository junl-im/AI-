...
세부 규칙은 [`VOICE_PRESET_FIDELITY.md`](VOICE_PRESET_FIDELITY.md)를 따릅니다.
...
4. 실제 모델·GPU 환경에서 5개 프리셋의 동일 문장과 장문 첫 구간을 각각 시험합니다.
5. 인물별 결과가 다르고 표시 성별과 맞는지 사람이 청취 확인합니다.
6. 실제 화자·장치 증거가 없으면 특정 성별 음색, 품질, 지연 성능을 보증 문구로 사용하지 않습니다.
