...
워크플로에는 `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`를 방어적으로 유지한다. 적용 뒤 Node.js 20 경고가 남으면 로그에 표시된 제3의 action 이름을 확인한다.
