...
1. 새 코드가 요구하는 모델 ID·버전·라이선스·SHA-256을 별도 매니페스트로 만든다.
2. 기존 모델 경로를 덮어쓰지 않고 새 버전 디렉터리에 병행 설치한다.
3. 동일 한국어 평가 세트와 실제 장치에서 CER·WER, RTF, 첫 음성 지연, VRAM·메모리를 비교한다.
4. canary 장치에서 Worker readiness와 장문 생성·재시도·Export를 통과한 뒤 기본 경로를 전환한다.
5. 이전 모델과 매니페스트는 최소 한 릴리스 동안 보존하고 환경 변수만 되돌려 rollback한다.
6. 라이선스 또는 체크섬이 달라지면 기존 동의를 재사용하지 않고 다시 명시적으로 확인한다.
